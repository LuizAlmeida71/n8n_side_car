...
def extrair_setor(text, lines):
    ...
    for i, line in enumerate(lines):
        if 'UNIDADE/SETOR:' in line.upper():
            # Primeiro tentar extrair da mesma linha
            match = re.search(r'UNIDADE/SETOR:\s*([^|\n]+?)(?=\s*ESCALA\s+DE\s+SERVI[CÇ]O:|$)', line, re.IGNORECASE)
            if match:
                setor = match.group(1).strip()
                # Limpar qualquer resto de "ESCALA DE SERVICO" que tenha ficado
                setor = re.sub(r'\s*ESCALA\s+DE\s+SERVI[CÇ]O:.*
...
async def normaliza_escala_maternidade_matricial(request: Request):
    try:
        ...
        return JSONResponse(content={"error": str(e)}, status_code=500), '', setor, flags=re.IGNORECASE).strip()
                return setor
            
            # Se não encontrou na mesma linha, verificar a próxima linha
            # (para casos onde o setor está na linha seguinte)
            if i + 1 < len(lines):
                next_line = lines[i + 1].strip()
                # Verificar se a próxima linha não é outro cabeçalho
                if next_line and not any(kw in next_line.upper() for kw in ['ESCALA', 'MÊS', 'ANO', 'NOME']):
...
async def normaliza_escala_maternidade_matricial(request: Request):
    try:
        ...
        return JSONResponse(content={"error": str(e)}, status_code=500), '', next_line, flags=re.IGNORECASE).strip()
                    if setor:
                        return setor
