...
def parse_mes_ano(text: str):
    month_regex = '|'.join(MONTH_MAP.keys())
    # Regex mais robusta para encontrar mês/ano
...
def interpretar_turno(token: str):
    ...
    # Prioriza turnos explícitos como 'PJ D', 'P N', 'D/N'
    token_clean = token.upper().replace('\n', ' ').strip()
    if 'D/N' in token_clean:
        return [{"turno": "MANHÃ"}, {"turno": "TARDE"}, {"turno": "NOITE"}]

    turnos = []
    if 'P' in token_clean or 'D' in token_clean: # Se tiver 'D' (Dia) ou 'P' (Plantão)
        turnos.extend([{"turno": "MANHÃ"}, {"turno": "TARDE"}])
    if 'N' in token_clean: # Se tiver 'N' (Noite)
        turnos.append({"turno": "NOITE"}) # Simplificado para noite inteira

    # Fallback para M/T se não encontrou D/N/P
    if not turnos:
...
def dedup_plantao(plantoes: list):
    ...
    result = []
    for p in plantoes:
        # Chave de dedup simplificada para dia e turno principal (Manhã/Tarde/Noite)
...
# --- Função Principal de Processamento de Página (Refatorada) ---

def processar_pagina_pdf(b64_content, page_info=""):
    try:
        pdf_bytes = base64.b64decode(b64_content)
        profissionais_encontrados = defaultdict(lambda: {
            "info": {},
            "plantoes": []
        })
        ...
        with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
            for i, page in enumerate(pdf.pages):
                # 1. LIMPEZA DO TEXTO: Remover as linhas indesejadas
                raw_text_lines = (page.extract_text() or "").split('\n')
                clean_lines = [
                    line for line in raw_text_lines
                    if not line.strip().startswith("Governo do Estado de Roraima") and \
                       not line.strip().startswith('"Amazônia: patrimônio dos brasileiros"')
                ]
                text = '\n'.join(clean_lines)

                mes, ano = parse_mes_ano(text)
                if not mes or not ano:
                    print(f"AVISO {page_info}-{i+1}: Mês/Ano não encontrado na página. Pulando.")
                    continue
                ...
                if setor_match:
                    nome_setor = (setor_match.group(1) or setor_match.group(2) or "").strip()
                else:
                    nome_setor = "NÃO INFORMADO"

                # 2. EXTRAÇÃO DE TABELA: Usar as tabelas extraídas pelo pdfplumber
                ...
                for table in tables:
                    ...
                    for row in table:
                        ...
                        # 3. MAPEAMENTO DE CABEÇALHO
                        is_header = any("NOME" in str(c or '').upper() for c in row)
                        if is_header and not header_map:
                            for col_idx, cell in enumerate(row):
                                ...
                                if "NOME" in cell_text: header_map["nome"] = col_idx
                                    ...
                                    day_match = re.match(r'^(\d{1,2})$', cell_text)
                                    if day_match:
                                        dia = int(day_match.group(1))
                                        header_map[dia] = col_idx
                                        dias_na_escala.append(dia)
                            continue # Próxima linha após encontrar cabeçalho

                        if not header_map: continue

                        # 4. PROCESSAMENTO DE LINHAS DE PROFISSIONAIS
                        nome_idx = header_map.get("nome")
                        if nome_idx is None or nome_idx >= len(row) or not row[nome_idx]:
                            continue
                        
                        nome_bruto = str(row[nome_idx]).replace('\n', ' ').strip()
                        
                        # Heurística para pular linhas que não são de profissionais
                        if len(nome_bruto.split()) < 2 or "TOTAL" in nome_bruto.upper():
                            continue

                        linha_completa = " ".join(filter(None, (str(c).strip() for c in row)))
                        if "PAES" not in linha_completa.upper() and "PJ" not in linha_completa.upper():
                            continue # Pula se não for PAES/PJ

                        # Agrupa os dados pelo nome do profissional
                        profissional = profissionais_encontrados[nome_bruto]
                        
                        if not profissional["info"]: # Salva informações na primeira vez que encontra o profissional
                           profissional["info"] = {
                                "medico_nome": nome_bruto,
                                "medico_crm": str(row[header_map.get("crm", -1)] or "N/I").strip(),
                                "medico_especialidade": str(row[header_map.get("cargo", -1)] or "N/I").strip(),
                                "medico_vinculo": str(row[header_map.get("vinculo", -1)] or "PJ").strip(),
                                "medico_setor": nome_setor,
                                "medico_unidade": nome_unidade,
                           }

                        # 5. EXTRAÇÃO DE PLANTÕES
        ...
        # 6. MONTAGEM FINAL
        lista_final = []
        for data in profissionais_encontrados.values():
            if data["plantoes"]:
                obj_final = data["info"]
                obj_final["plantoes"] = dedup_plantao(data["plantoes"])
                lista_final.append(obj_final)
...
# Endpoint (seu código original, adaptado para usar a nova função)
...
async def normaliza_escala_MATERNIDADE_MATRICIAL(request: Request):
    ...
    # Agrupar profissionais com o mesmo nome que podem ter vindo de arquivos diferentes
    profissionais_agrupados = defaultdict(lambda: {"info": {}, "plantoes": []})
    for prof in todos_profissionais:
        nome = prof["medico_nome"]
        if not profissionais_agrupados[nome]["info"]:
            profissionais_agrupados[nome]["info"] = prof
        profissionais_agrupados[nome]["plantoes"].extend(prof["plantoes"])
    
    lista_final = []
    for data in profissionais_agrupados.values():
        prof_obj = data["info"]
        prof_obj["plantoes"] = dedup_plantao(data["plantoes"])
        prof_obj["plantoes"].sort(key=lambda x: datetime.strptime(x['data'], '%d/%m/%Y'))
        lista_final.append(prof_obj)

    lista_final.sort(key=lambda p: p["medico_nome"])

    return JSONResponse(content=[{
        "unidade_escala": "MISTA",
        "mes_ano_escala": "JUNHO/2025", # Pode ser melhorado para extrair dinamicamente
        "profissionais": lista_final
    }])
